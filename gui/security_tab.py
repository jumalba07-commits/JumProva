...
class SecurityTab:
    ...
    def _build_ui(self):
        ...
        frame_instrucciones = ttk.LabelFrame(
            main_frame, text="🛡️ Cazador de malware", padding=5)
        ...
        self.btn_matar_proceso = ttk.Button(
            frame_botones,
            text="💀 MATAR PROCESO",
        ...
        self.btn_eliminar_archivo = ttk.Button(
            frame_botones,
        ...
        self.label_resumen = ttk.Label(
            frame_resumen,
            text="🔍 Haz clic en 'Analizar' para escanear tu sistema.")
        ...
        frame_tabla = ttk.LabelFrame(
            main_frame, text="📋 Resultados encontrados", padding=2)
        ...
        frame_progreso = ttk.LabelFrame(
            main_frame, text="📊 Progreso", padding=2)
            ...
            try:
                ...
                for proc in psutil.process_iter(
                    ...
                    if self.cancelar_analisis:
                        self._log(
                            "⛔ Análisis cancelado por el usuario", "warning")
                        break
                    ...
                    try:
                        ...
                        if procesados > 5:
                            ...
                            eta = (tiempo_total / procesados) * \
                                (total_procesos - procesados)
                            if eta < 60:
                                eta_str = f"{int(eta)}s"
                            else:
                        ...
                        self.parent.after(
                            ...
                            lambda p=progress,
                            n=nombre,
                            e=eta_str: self._update_progress(
                                p,
                                f"📄 {n}",
                                e))

                        riesgo, detalles = self._evaluar_riesgo(
                            nombre, exe, cmdline)
                        ...
                        if riesgo > 0:
                            ...
                            self.parent.after(
                                0, lambda: self._log(
                                    f"⚠️ {nombre} (PID: {pid}) - {detalles}", "warning"))

                    except BaseException:
                        continue

                self.parent.after(
                    0, lambda: self._mostrar_procesos(
                        procesos, sospechosos_encontrados))
                self.parent.after(
                ...
                self.parent.after(
                    0,
                    lambda: self._log(
                        "❌ psutil no instalado. Ejecuta: pip install psutil",
                        "error"))
                self.parent.after(
                    0, lambda: self._update_progress(
                        0, "Error: psutil no instalado"))
                ...
                self.parent.after(
                    0, lambda: self.btn_analizar.config(
                        state="normal"))
                self.parent.after(
                    0, lambda: self.btn_revisar.config(
                        state="normal"))
                self.parent.after(
                    0, lambda: self.btn_red.config(
                        state="normal"))
                self.parent.after(
    ...
    def _mostrar_procesos(self, procesos, sospechosos):
        ...
        if not procesos:
            ...
            self.label_resumen.config(
                text="✅ No se encontraron procesos sospechosos. ¡Sistema limpio!")
            return
        ...
        for proc in procesos:
            ...
            if tag == "alto":
                self.tree.tag_configure(
                    'alto', background='#ffebee')  # Rojo claro
            elif tag == "medio":
                self.tree.tag_configure(
                    'medio', background='#fff8e1')  # Amarillo claro
            elif tag == "bajo":
                self.tree.tag_configure(
                    'bajo', background='#fff3e0')  # Naranja claro

            self.tree.item(item, tags=(tag,))

        self._log(f"📊 Procesos analizados: {len(procesos)}", "info")
        self._log(
            f"⚠️ Sospechosos encontrados: {sospechosos}",
            "warning" if sospechosos > 0 else "success")

        if sospechosos > 0:
            self.label_resumen.config(
                text=f"⚠️ Se encontraron {sospechosos} procesos sospechosos. Revisa la tabla.")
        else:
            self.label_resumen.config(
                text="✅ No se encontraron procesos sospechosos.")
    ...
    def _matar_proceso_seleccionado(self):
        ...
        try:
            ...
            try:
                proc.terminate()
                self._log(
                    f"⏳ Terminando proceso: {nombre} (PID: {pid})", "info")
                time.sleep(1)
            except BaseException:
                pass
            ...
            if proc.is_running():
                proc.kill()
                self._log(
                    f"💀 Proceso forzado: {nombre} (PID: {pid})",
                    "warning")
    ...
    def _eliminar_archivo_seleccionado(self):
        ...
        try:
            ...
            if not archivo_encontrado:
                messagebox.showinfo(
                    "Info", f"No se encontró el archivo:\n{nombre}")
                return

            # Verificar si está en carpeta de Windows
            if str(archivo_encontrado).lower().startswith('c:\\windows') or str(
                    archivo_encontrado).lower().startswith('c:\\program files'):
                messagebox.showerror("⛔ SEGURIDAD",
                                     f"❌ NO SE PUEDE ELIMINAR\n\n"
            ...
            error_msg = str(e)
            self._log(f"❌ Error al eliminar {nombre}: {error_msg}", "error")
            messagebox.showerror(
                "Error",
        ...
        def revisar():
            try:
                ...
                self.parent.after(
                    0,
                    lambda: self._log(
                        f"✅ Revisión completada. Archivos sospechosos encontrados: {archivos_encontrados}",
                        "info"))
                self.parent.after(0, lambda: self.label_resumen.config(
                    text=f"📁 Archivos sospechosos encontrados: {archivos_encontrados}"))
                self.parent.after(
                    0, lambda: self._update_progress(
                        100, "✅ Revisión completada"))
    ...
    def _revisar_conexiones(self):
        ...
        def revisar():
            try:
                ...
                puertos_sospechosos = {
                    4444, 6667, 1337, 31337, 5555, 12345, 31337}
                _ = []
                ...
                if conexiones:
                    ...
                    self._log(
                        f"📊 Total de conexiones activas: {
                            len(conexiones)}", "info")

                    if conexiones_sospechosas:
                        self._log(
                            f"⚠️ Conexiones sospechosas encontradas: {
                                len(conexiones_sospechosas)}", "warning")
                        ...
                        self._log(
                            "✅ No se encontraron conexiones sospechosas", "success")
                    ...
                    if conexiones_sospechosas:
                        resumen += f" | ⚠️ {
                            len(conexiones_sospechosas)} sospechosas"
                    else:
                        resumen += " | ✅ Todo seguro"
                    ...
                    if conexiones_sospechosas:
                        mensaje += f"⚠️ Conexiones sospechosas: {
                            len(conexiones_sospechosas)}\n\n"
