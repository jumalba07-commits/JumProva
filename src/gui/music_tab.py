"""
Pestaña para organizar música - Diseño tipo Limpieza/Metadatos.
"""
import threading
...
from ..core.music_organizer import MusicOrganizer


class MusicTab(ttk.Frame):
    """Pestaña de organización de música con diseño tipo Limpieza."""
    ...
    def _create_widgets(self):
        ...
        ttk.Label(text_frame, text="🎵 Organizar Música", style='Title.TLabel').pack(anchor="w")
        ttk.Label(text_frame, text="Organiza tu biblioteca musical por Artista / Álbum / Año", style='Subtitle.TLabel').pack(anchor="w")
        ...
        ttk.Button(dest_btn_frame, text="↩️ Usar mismo", command=self._use_same_destination, style='Secondary.TButton', width=10).pack(side="left", padx=(0, 5))
        ttk.Button(dest_btn_frame, text="📂 Examinar", command=self._select_destination, style='Secondary.TButton', width=10).pack(side="left")
        ...
        columns = ("Archivo", "Artista", "Álbum", "Año", "Acción")
        ...
        self.tree.column("Artista", width=120, anchor="w")
        self.tree.column("Álbum", width=120, anchor="w")
    ...
    def _organize_files(self):
        ...
        self._log("🎵 INICIANDO ORGANIZACIÓN DE MÚSICA...")
        ...
        self._log(f"📋 Usar metadatos: {'Sí' if self.use_metadata_var.get() else 'No'}")
        self._log(f"📝 Analizar nombre: {'Sí' if self.parse_filename_var.get() else 'No'}")
        self._log(f"📂 Estructura: {self.order_var.get().replace('_', ' / ')}")
        self._log("-" * 40)
    ...
    def _run_organizer(self):
        try:
            ...
            summary = self.organizer.organize(
                ...
                parse_filename=self.parse_filename_var.get(),
    ...
    def _on_organize_complete(self, summary: dict):
        ...
        if summary.get('cancelled', False):
            ...
            self._log(f"✅ ¡ORGANIZACIÓN COMPLETADA!")
