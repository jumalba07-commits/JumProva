...
class Organizer:
    ...
    def organize(self, source_dir, dest_dir, operation="move"):
        ...
        if not files:
            if self.callbacks["on_log"]:
                self.callbacks["on_log"]("⚠️ No se encontraron archivos para organizar")
        ...
        for file_path in files:
            ...
            if self.cancel_requested:
                if self.callbacks["on_log"]:
                    self.callbacks["on_log"]("🛑 Proceso cancelado por el usuario")
            ...
            try:
                ...
                if category["name"] == "otros" and self.handle_unknown == "ignore":
                    if self.callbacks["on_file_processed"]:
                        self.callbacks["on_file_processed"](file_path, "ignored", "Archivo desconocido ignorado")
                    if self.callbacks["on_log"]:
                        self.callbacks["on_log"]("   ⏭️ Archivo ignorado (categoría desconocida)")
                    continue
                
                # Obtener fecha
                use_metadata = category.get("use_metadata", False) and self.use_metadata
                date = self.date_detector.get_date(file_path, use_metadata)
                
                # Extraer componentes de fecha
                date_components = self.date_detector.extract_date_components(date)
                if self.callbacks["on_log"]:
                    self.callbacks["on_log"](f"   📅 Fecha: {date_components['year']}/{date_components['month']:02d}-{date_components['month_name']}/{date_components['day']:02d}")
                ...
                result = self.file_handler.move_file(file_path, dest_path, operation)
                ...
                if self.callbacks["on_file_processed"]:
                    self.callbacks["on_file_processed"](
                        file_path, 
                        "processed",
                        f"→ {category['folder']}/{date_components['year']}/{date_components['month']:02d}-{date_components['month_name']}/{date_components['day']:02d}"
                    )
                
                if self.callbacks["on_progress"]:
                    progress = (processed / len(files)) * 100
                    self.callbacks["on_progress"](progress, f"Procesando: {file_path.name}")
    ...
    def _build_dest_path(self, dest_dir, category_folder, date_components, file_path):
                ...
                if audio and hasattr(audio, 'tags') and audio.tags is not None:
                    if audio_organize == "artista" and 'TPE1' in audio.tags:
                        ...
                        if artista:
                            ...
                            dest_path = dest_dir / category_folder / artista / str(year) / f"{month_num:02d}-{month_name}" / f"{day:02d}"
                        ...
                        if genero:
                            dest_path = dest_dir / category_folder / genero / str(year) / f"{month_num:02d}-{month_name}" / f"{day:02d}"
                            dest_path = dest_path / file_path.name
                            return dest_path
            except:
                pass
        ...
        dest_path = dest_dir / category_folder / str(year) / f"{month_num:02d}-{month_name}" / f"{day:02d}"
        dest_path = dest_path / file_path.name
