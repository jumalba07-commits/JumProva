...
class DesorganizeTab:
    ...
    def _build_ui(self):
        ...
        frame_instrucciones = ttk.LabelFrame(
            top_frame, text="📋 Instrucciones", padding=8)
        ...
        # ---- Carpetas ----
        frame_carpetas = ttk.LabelFrame(
            top_frame, text="📁 Carpetas", padding=10)
        ...
        ttk.Button(
            frame_carpetas,
            text="📂",
            command=lambda: self._select_folder("source")).grid(
            row=0,
            column=2)
        ...
        ttk.Button(
            frame_carpetas,
            text="📂",
            command=lambda: self._select_folder("dest")).grid(
            row=1,
            column=2)
        ...
        frame_opciones = ttk.LabelFrame(
            top_frame, text="⚙️ Opciones", padding=10)
        ...
        ttk.Checkbutton(
            frame_opciones,
            text="📂 Incluir subcarpetas (recursivo)",
            variable=self.include_subfolders).grid(
            row=1,
            column=0,
            columnspan=3,
            sticky="w",
        ...
        ttk.Checkbutton(
            frame_opciones,
            text="📁 Mantener estructura en nombres (ej: carpeta_archivo.jpg)",
            variable=self.keep_structure).grid(
            row=2,
            column=0,
            columnspan=3,
            sticky="w",
        ...
        ttk.Checkbutton(
            frame_opciones,
            text="📷 Solo fotos y videos (ignorar otros archivos)",
            variable=self.only_media).grid(
            row=3,
            column=0,
            columnspan=3,
            sticky="w",
        ...
        ttk.Checkbutton(
            frame_opciones,
            text="🗑️ Eliminar carpetas vacías después",
            variable=self.delete_empty).grid(
            row=4,
            column=0,
            columnspan=3,
            sticky="w",
        ...
        self.btn_clear_desorganize = ttk.Button(
            frame_botones,
            text="🧹 LIMPIAR TODO",
            command=self._clear_all_desorganize)
        self.btn_clear_desorganize.pack(
            side=tk.RIGHT, padx=5, fill="x", expand=True, ipady=3)

        # Botón DESORGANIZAR
        self.btn_desorganize = ttk.Button(frame_botones, text="🔄 DESORGANIZAR",
                                          command=self._start_desorganize,
                                          style="Accent.TButton")
        self.btn_desorganize.pack(
            side=tk.RIGHT,
            padx=5,
            fill="x",
            expand=True,
        ...
        frame_progreso = ttk.LabelFrame(
            bottom_frame, text="📊 Progreso", padding=8)
    ...
    def _use_same_folder(self):
        ...
        if source:
            ...
            messagebox.showwarning(
                "Advertencia",
                "Primero selecciona una carpeta origen")
    ...
    def _scan_folder(self):
        ...
        def scan():
            try:
                ...
                self.parent.after(0, lambda: self._show_scan_results(
                    file_list, total_files, total_size, folder_path))
                self.parent.after(
                ...
                error_msg = str(e)
                self.parent.after(
                    0, lambda: self._log_message(
                        f"❌ Error al escanear: {error_msg}", "error"))
                self.parent.after(
                    0, lambda: self.btn_scan.config(
                        state="normal"))
    ...
    def _show_scan_results(
        ...
        if total_files > 0:
            ...
            for i, file_info in enumerate(file_list[:50]):
                ...
                size_str = f"{
                    size_kb:.1f} KB" if size_kb < 1024 else f"{
                    size_kb / 1024:.1f} MB"
                self._log_message(
                    f"   {
                        i +
        ...
        self._log_message("", "info")
        self._log_message(
            "💡 Presiona 'DESORGANIZAR' para procesar todos los archivos",
            "info")
    ...
    def _start_desorganize(self):
        ...
        self._log_message(
            f"🔄 {
                'Mover' if self.operation.get() == 'move' else 'Copiar'}",
            "info")
        self._log_message("=" * 60, "info")
        ...
        def run():
            try:
                ...
                self.parent.after(
                    0, lambda: self.btn_desorganize.config(
                        state="normal"))
                self.parent.after(
                    0, lambda: self.btn_scan.config(
                        state="normal"))
                self.parent.after(
                    0, lambda: self.label_progress.config(
                        text="✅ Proceso completado"))
                self.parent.after(
                    0, lambda: self.progress_bar.config(
                        value=100))
        ...
        for file_path in files:
            ...
            try:
                ...
                self.parent.after(
                    0,
                    lambda p=progress,
                    n=file_path.name: self._update_progress(
                        p,
                        n))
