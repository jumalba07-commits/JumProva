...
class SpaceLiberatorTab:
    ...
    def _build_ui(self):
        ...
        ttk.Entry(frame_fila1, textvariable=self.target_dir, width=45).pack(side=tk.LEFT, padx=2)
        ttk.Button(frame_fila1, text="📂", command=lambda: self._select_folder(), width=3).pack(side=tk.LEFT, padx=2)
        
        ttk.Checkbutton(frame_fila1, text="📂 Subcarpetas",
                       variable=self.include_subfolders).pack(side=tk.LEFT, padx=10)

        # ---- ADVERTENCIA DE SEGURIDAD (creativa) ----
        warning_frame = tk.Frame(main_frame, bg="#fff3cd", relief=tk.RAISED, bd=2)
        ...
        tk.Label(icon_frame, text="⚠️", font=("Arial", 20), bg="#fff3cd").pack()
        ...
        tk.Label(text_frame, 
                ...
                font=("Arial", 10, "bold"), fg="#856404", bg="#fff3cd", anchor="w").pack(anchor="w")
        
        tk.Label(text_frame, 
                text="💡 Este liberador es inteligente, pero no adivina. Si no sabes qué es un archivo, NO lo borres.",
                font=("Arial", 8), fg="#856404", bg="#fff3cd", anchor="w").pack(anchor="w")
        
        # ============================================
        # FILA 2: Opciones de limpieza (en columnas)
        # ============================================
        frame_fila2 = ttk.LabelFrame(main_frame, text="🧹 Qué limpiar", padding=5)
        ...
        ttk.Spinbox(frame_spin1, from_=1, to=10000, textvariable=self.large_size_mb, width=8).pack(side=tk.LEFT)
        ...
        ttk.Spinbox(frame_spin2, from_=1, to=365, textvariable=self.old_days, width=8).pack(side=tk.LEFT)
        ...
        ttk.Spinbox(frame_spin3, from_=1, to=100000, textvariable=self.min_duplicate_size_kb, width=8).pack(side=tk.LEFT)
        ...
        ttk.Radiobutton(frame_accion, text="📁 Mover a basura", variable=self.action, 
                       value="move_trash").pack(side=tk.LEFT, padx=5)
        ...
        # ---- Resumen (compacto) ----
        frame_resumen = ttk.LabelFrame(bottom_frame, text="📊 Resumen", padding=2)
        frame_resumen.pack(fill="x", pady=1)
        
        self.label_resumen = ttk.Label(frame_resumen, text="🔍 Analiza una carpeta para ver qué espacio puedes liberar.")
        self.label_resumen.pack(anchor="w", padx=5)
        
        # ---- TABLA DE RESULTADOS (arriba) ----
        frame_tabla = ttk.LabelFrame(bottom_frame, text="📋 Resultados encontrados", padding=2)
        ...
        frame_progreso = ttk.LabelFrame(bottom_frame, text="📊 Progreso", padding=2)
    ...
    def _cancelar_limpieza(self):
        ...
        if messagebox.askyesno("⛔ Cancelar limpieza", 
            ...
            self._update_progress(self.progress_bar.cget('value'), "⛔ Cancelando...")
    ...
    def _es_archivo_protegido(self, file_path):
        ...
        for carpeta in self.carpetas_usuario_protegidas:
            if f'\\{carpeta.lower()}\\' in file_str or file_str.endswith(f'\\{carpeta.lower()}'):
    ...
    def _analizar_espacio(self):
        ...
        if not any([
            ...
            messagebox.showwarning("Advertencia", "Selecciona al menos una opción de limpieza")
            return
        ...
        def analizar():
            try:
                ...
                if self.include_subfolders.get():
                    ...
                    all_folders = [p for p in folder_path.rglob("*") if p.is_dir()]
                else:
                    all_files = list(folder_path.iterdir())
                    all_folders = [p for p in folder_path.iterdir() if p.is_dir()]
                
                total_files = len([f for f in all_files if f.is_file()])
                
                # Calcular tareas para el progreso
                tareas = 0
                if self.clean_temp.get(): tareas += 1
                if self.clean_duplicates.get(): tareas += 1
                if self.clean_empty.get(): tareas += 1
                if self.clean_large.get(): tareas += 1
                if self.clean_old.get(): tareas += 1
                if self.clean_logs.get(): tareas += 1
                if self.clean_recycle.get(): tareas += 1
                if self.clean_cache.get(): tareas += 1
                ...
                if self.clean_temp.get():
                    ...
                    self.parent.after(0, lambda: self._log("🔍 Buscando archivos temporales...", "info"))
                    temp_count = 0
                    ...
                    for i, file_path in enumerate(all_files):
                        ...
                        if i % 50 == 0:
                            progreso_tarea = (i / total_files) * progreso_por_tarea
                            progreso_total = progreso_base + progreso_tarea
                            self.parent.after(0, lambda p=progreso_total: self._update_progress(p, f"📄 {file_path.name}"))
                    
                    self.parent.after(0, lambda: self._log(f"   ✅ Temporales: {temp_count}", "success"))
                
                # 2. DUPLICADOS
                if self.clean_duplicates.get():
                    tarea_actual += 1
                    progreso_base = (tarea_actual - 1) * progreso_por_tarea
                    self.parent.after(0, lambda: self._log("🔍 Buscando archivos duplicados...", "info"))
                    size_map = {}
                    dup_count = 0
                    ...
                    self.parent.after(0, lambda: self._log(f"   ✅ Duplicados: {dup_count}", "success"))
                
                # 3. CARPETAS VACÍAS
                if self.clean_empty.get():
                    tarea_actual += 1
                    progreso_base = (tarea_actual - 1) * progreso_por_tarea
                    self.parent.after(0, lambda: self._log("🔍 Buscando carpetas vacías...", "info"))
                    empty_count = 0
                    ...
                    self.parent.after(0, lambda: self._log(f"   ✅ Carpetas vacías: {empty_count}", "success"))
                
                # 4. ARCHIVOS GRANDES
                if self.clean_large.get():
                    tarea_actual += 1
                    progreso_base = (tarea_actual - 1) * progreso_por_tarea
                    self.parent.after(0, lambda: self._log(f"🔍 Buscando archivos grandes (+{self.large_size_mb.get()}MB)...", "info"))
                    large_count = 0
                    ...
                    self.parent.after(0, lambda: self._log(f"   ✅ Grandes: {large_count}", "success"))
                
                # 5. ARCHIVOS ANTIGUOS
                if self.clean_old.get():
                    tarea_actual += 1
                    progreso_base = (tarea_actual - 1) * progreso_por_tarea
                    self.parent.after(0, lambda: self._log(f"🔍 Buscando archivos antiguos (+{self.old_days.get()} días)...", "info"))
                    old_count = 0
                    ...
                    self.parent.after(0, lambda: self._log(f"   ✅ Antiguos: {old_count}", "success"))
                
                # 6. LOGS
                if self.clean_logs.get():
                    tarea_actual += 1
                    progreso_base = (tarea_actual - 1) * progreso_por_tarea
                    self.parent.after(0, lambda: self._log("🔍 Buscando logs y depuración...", "info"))
                    log_count = 0
                    ...
                    self.parent.after(0, lambda: self._log(f"   ✅ Logs: {log_count}", "success"))
                
                # 7. PAPELERA
                if self.clean_recycle.get():
                    tarea_actual += 1
                    progreso_base = (tarea_actual - 1) * progreso_por_tarea
                    self.parent.after(0, lambda: self._log("🔍 Buscando papelera de reciclaje...", "info"))
                    recycle_count = 0
                    ...
                    for i, file_path in enumerate(all_files):
                        ...
                        if '$recycle.bin' in str(file_path).lower() or 'recycler' in str(file_path).lower():
                            self.current_files.append({
                                "path": file_path,
                                "name": file_path.name,
                                "size": file_path.stat().st_size if file_path.exists() else 0,
                    ...
                    self.parent.after(0, lambda: self._log(f"   ✅ Papelera: {recycle_count}", "success"))
                
                # 8. CACHÉS
                if self.clean_cache.get():
                    tarea_actual += 1
                    progreso_base = (tarea_actual - 1) * progreso_por_tarea
                    self.parent.after(0, lambda: self._log("🔍 Buscando cachés...", "info"))
                    cache_count = 0
                    cache_patterns = ['cache', 'thumbcache', 'thumbnail', 'preview']
                    ...
                    self.parent.after(0, lambda: self._log(f"   ✅ Cachés: {cache_count}", "success"))
    ...
    def _mostrar_resultados(self):
        ...
        if not all_items:
            ...
            self.label_resumen.config(text="✅ No se encontraron archivos para limpiar. ¡Tu sistema está limpio!")
            self.btn_clean.config(state="disabled")
            self.btn_informe.config(state="disabled")
        ...
        resumen = f"📊 {len(all_items)} archivos | 📦 {size_str} | ⏱️ {tiempo_str}"
        ...
        try:
            with open(archivo_informe, 'w', encoding='utf-8') as f:
                ...
                for item in items:
                    ...
                    try:
                        if 'GB' in tamaño_str:
                            total_size += float(tamaño_str.replace('GB', '').strip()) * 1024 * 1024 * 1024
                        elif 'MB' in tamaño_str:
                            total_size += float(tamaño_str.replace('MB', '').strip()) * 1024 * 1024
                        elif 'KB' in tamaño_str:
                            total_size += float(tamaño_str.replace('KB', '').strip()) * 1024
                    except:
                        pass
                ...
                f.write("=" * 80 + "\n")
                f.write("Este informe muestra los archivos que el liberador de espacio ha encontrado.\n")
                f.write("Antes de eliminarlos, asegúrate de que no son necesarios para tu sistema.\n")
                f.write("Revisa la ubicación de cada archivo antes de proceder con la limpieza.\n")
                f.write("=" * 80 + "\n")
            
            self._log(f"✅ Informe guardado: {archivo_informe}", "success")
            messagebox.showinfo("Éxito", f"✅ Informe guardado correctamente\n\n{archivo_informe}")
            
            if messagebox.askyesno("Abrir informe", "¿Quieres abrir el archivo de informe ahora?"):
    ...
    def _liberar_espacio(self):
        ...
        for item in items:
            ...
            if len(values) >= 2:
                ...
                try:
                    if 'GB' in size_str:
                        total_size += float(size_str.replace('GB', '').strip()) * 1024 * 1024 * 1024
                    elif 'MB' in size_str:
                        total_size += float(size_str.replace('MB', '').strip()) * 1024 * 1024
                    elif 'KB' in size_str:
        ...
        def liberar():
            ...
            for i, item in enumerate(items_list):
                ...
                if self.cancelar_limpieza:
                    ...
                    self._log(f"⛔ Limpieza cancelada por el usuario", "warning")
                ...
                self.parent.after(0, lambda p=progress, n=nombre_archivo: self._update_progress(p, f"🧹 {n}"))
                self.parent.after(0, lambda e=eta: self.label_eta.config(text=f"⏱️ {e}"))
                
                try:
                    # Buscar el archivo
                    file_path = None
                    for file_info in self.current_files + self.duplicates + self.empty_folders + self.temp_files:
                    ...
                    if es_protegido:
                        protegidos += 1
                        self._log(f"🔴 {nombre_archivo} - PROTEGIDO: {razon}", "error")
                    ...
                    if self.action.get() == "delete":
                        ...
                        if messagebox.askyesno("Eliminar", f"¿Eliminar {file_path.name}?"):
                            if file_path.is_dir():
                                shutil.rmtree(str(file_path))
                            else:
                                os.remove(str(file_path))
            ...
            # 5. FINALIZAR
            self.parent.after(0, lambda: self._update_progress(100, "✅ Proceso finalizado"))
            self._log("", "info")
            self._log("=" * 50, "bold")
            ...
            if self.cancelar_limpieza:
                ...
                self._log(f"❌ Errores: {errores}", "error" if errores > 0 else "info")
            else:
            ...
            self.parent.after(0, lambda: self.label_resumen.config(text=resumen_msg))
            ...
            if self.cancelar_limpieza:
                ...
                if errores > 0:
                    msg += f"\n❌ Errores: {errores}"
                self.parent.after(0, lambda: messagebox.showinfo("⛔ Cancelado", msg))
            else:
            ...
            # Habilitar botones
            self.parent.after(0, lambda: self.btn_clean.config(state="disabled"))
            self.parent.after(0, lambda: self.btn_scan.config(state="normal"))
            self.parent.after(0, lambda: self.btn_informe.config(state="normal"))
            self.parent.after(0, lambda: self.btn_cancel.config(state="disabled"))
            self.parent.after(0, lambda: setattr(self, 'limpiando', False))
            self.parent.after(0, lambda: setattr(self, 'cancelar_limpieza', False))
