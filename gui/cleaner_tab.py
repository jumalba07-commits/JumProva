...
class CleanerTab:
    ...
    def _build_ui(self):
        ...
        frame_folder = ttk.LabelFrame(
            self.parent, text="📂 Carpeta a limpiar", padding=10)
        ...
        ttk.Button(
            frame_folder,
            text="📂",
            command=self._select_folder).grid(
            row=0,
            column=2)

        # ===== Opciones =====
        frame_opciones = ttk.LabelFrame(
            self.parent, text="⚙️ Opciones de limpieza", padding=10)
        frame_opciones.pack(fill="x", pady=5)

        # Tamaño mínimo
        ttk.Label(
            frame_opciones,
            text="Tamaño mínimo (KB):").grid(
            row=0,
            column=0,
            sticky="w")
        ttk.Spinbox(
        ...
        ttk.Label(
            frame_opciones,
            text="(archivos más pequeños serán considerados basura)").grid(
            row=0,
            column=2,
            sticky="w",
            padx=5)
        ...
        frame_resultados = ttk.LabelFrame(
            self.parent, text="📋 Resultados del escaneo", padding=10)
        ...
        frame_stats = ttk.LabelFrame(
            self.parent, text="📊 Estadísticas", padding=10)
        frame_stats.pack(fill="x", pady=5)

        self.label_stats = ttk.Label(
            frame_stats, text="ℹ️ Esperando escaneo...")
            ...
            try:
                ...
                self.parent.after(
                    0, lambda: messagebox.showerror(
                        "Error", f"Error al escanear:\n{error_msg}"))
                self.parent.after(
                    0, lambda: self.label_stats.config(
...
🗑️ Listos para limpiar
        ...
        if stats['total_detected'] > 0:
            ...
            messagebox.showinfo(
                "✅ Escaneo completado", f"Se encontraron {
                    stats['total_detected']} archivos basura\n" f"📦 Tamaño total: {
                    total_size:.1f} KB\n\n" f"💡 Puedes revisar la lista y luego limpiar.")
        else:
            messagebox.showinfo(
                "✅ Escaneo completado",
                "🎉 No se encontraron archivos basura")
    ...
    def _clean_junk(self):
        ...
        action_text = "eliminar permanentemente" if self.action.get(
        ) == "delete" else "mover a la carpeta Basura"
        ...
        def clean():
            try:
                if self.action.get() == "delete":
                    ...
                    msg = f"✅ Eliminados permanentemente: {
                        len(
                            result['deleted'])} archivos"
                else:
                    result = self.cleaner.move_to_recycle(self.junk_files)
                    msg = f"✅ Movidos a 'Basura': {
                        len(
                            result['moved'])} archivos"
                ...
                self.parent.after(
                    0, lambda: messagebox.showinfo(
                        "✅ Limpieza completada", msg))
                self.parent.after(0, self._scan_junk)  # Rescanear
            except Exception as e:
                error_msg = str(e)
                self.parent.after(
                    0, lambda: messagebox.showerror(
                        "Error", f"Error en la limpieza:\n{error_msg}"))
                self.parent.after(
